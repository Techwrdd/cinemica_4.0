...
from info import VERIFY_SHORTNER, MALTI_BUTTONS, SHORT_URL2, SHORT_API2, SHORTENER, LONG_DROPLINK_URL, DIRECT_GEN, DIRECT_GEN_DB, DIRECT_GEN_URL, AUTH_CHANNEL, VERIFY_1_SHORTENERS, LONG_IMDB_DESCRIPTION, MAX_LIST_ELM, SHORT_URL, SHORTENER_API, SHORTENER_API2, SHORTENER_WEBSITE, SHORTENER_WEBSITE2
...
import shortzy 
from plugins.stream import stream_short_link
import re
import os
...
class temp(object):
    ...
    B_LINK = None
    SETTINGS = {}
    multi_buttons = int(MALTI_BUTTONS)
    BATCH_FILES = {}
...
async def get_shortlink(url, is_second_shortener=False, verify_1=False):
    if VERIFY_SHORTNER:
        if is_second_shortener:
            api, site = SHORTENER_API2, SHORTENER_WEBSITE2
        elif verify_1:
            for api, site in VERIFY_1_SHORTENERS:
                url = await short_link_func(site, api, url)
            return url
        else:
...
async def short_link_func(site, api, url):
